...
public class ProgressCoordenateDiff extends SwingWorker<Integer, String> {
	...
	public String[] columnNameCoordenadas = { "UPM", "Lat Teorica", "Long Teorica", "Lat Capturada", "Long Capturada",
			"Diferencia m:" , "Tipo UPM","Evidencia_Muestreo","Error presición"};
	...
	public void getCoordenates(String ruta) {
		...
		try {
			...
			while (rsExterno.next()) {
				...
				System.out.println("COORDENADAS GEOGRAFICAS\tlat_teo_string ="+lat_teo_string +"\tlong_teo_string ="+long_teo_string +"\tlat_cap_string ="+lat_cap_string +"\tlong_cap_string ="+long_cap_string);
				System.out.println("COORDENADAS DECIMALES\tlat_teo_flot ="+lat_teo_flot +"\tlong_teo_flot ="+long_teo_flot +"\tlat_cap_flot ="+lat_cap_flot +"\tlong_cap_flot ="+long_cap_flot );
				...
				if (diferencia > 12.55) {
					CoordenadasModel.addRow(new Object[] { UPMID, lat_teo_string, "-" + long_teo_string, lat_cap_string,
							long_cap_string, diferencia ,tipo_upm,incial_remplazo,erro_presicion});
				}
